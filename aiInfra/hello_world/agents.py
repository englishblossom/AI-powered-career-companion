...
from latex_modi import genResume

load_dotenv()
from latex_format import gen_latex_resume


# add scheduling and all other regular tasks like pointing where it can be found in our website and steps to take
def get_repos(url: str) -> str | dict[Any, Any]:
    """Gets public repositories for a given GitHub username."""
    ...
    repos = response.json()
    def fetch_and_analyze_files(file_urls):
        contents = []
    ...
    def list_repo_files(user, path=""):
        url = f"{user}/{path}" if path else f"{user}/contents"
        response = requests.get(url)
        if response.status_code != 200:
            return ""
        ...
        for item in response.json():
            if item["type"] == "file" and item["name"].endswith((".py", ".js", ".java", ".ts", ".go",".ipynb")):
                files.append(item["download_url"])
                print(item["download_url"])
                ret_str += fetch_and_analyze_files(item["download_url"]) + "\n\n"
            elif item["type"] == "dir":
                files.extend(list_repo_files(url, item["path"]))
        return ret_str
    ret = {}
    for repo in repos:
        files = list_repo_files(repo["url"])
        ret[repo["name"]] = files
    return ret
...
def function_get_relevant_projects(name:str,job_description:str,urls:List[str],job_title:str,num_iters=10) -> str:
    projects = [get_repos(url_) for url_ in urls]
    print(projects)
    prompt = """
    Extract any projects from the provided projects that might be relevant to the user for making his resume great.
    input consists of a list of projects with repo name and the code next to it.
    For each repo, check if the project is relevant to the job description provided and return projects even if they deal with an adjacent idea to the provided search query. Err on the side of verbosity.
    Scope of the search is limited to the provided projects.
     [STRICTLY USE THE PROVIDED projects]
    
    given the set of projects
    <projects>
    {projects}
    </projects>
    ...
    {{"projects": [
            "project_title": "",
            "points": [points about highlighting the project and making it relevant to the job description, should be between 3-5 points and each point should be between 140-150 characters",]
            "link": ""
            ], ...
        }},
    </output_format>
    ...
    try:
        ...
        agent = {"input": lambda x: x["input"],
                 ...
                 "projects": lambda x: x["projects"]} | prompt | llm | ReActSingleInputOutputParser()
...
def function_get_profile_keywords(name:str,
    ...
    Question: the input question you must answer
    Thought: you should always think about what to do to resolve the user question
    Action: the action to take, should be one of [{tool_names}] [STRICTLY USE THE TOOL NAMES], refrain from using any other tool names
    Action Input: the input to the action 
    Observation: the result of the action
    ... (this Thought/Action/Action Input/Observation can repeat N times)
    Thought: I now know the final answer
    Final Answer:   Final answer should be in the following format do not append any other text, just return the keywords in the following format:
    <output_format>
    Technical Keywords:
        - <keyword>
        - <keyword>
...
def function_resume_feedback(num_iters: int,resume:str,job_description:str,company_name:str) -> str:
    ...
    AS well as a list of 5-8 points critiquing the resume and providing feedback on how to improve it, strongly focus on critiquing the resume focus on negatives and how can you improve the score so that this feedback can be used by an another llm.
    The points should be written in such a way that it can be used by another llm to finetune the resume.
...
def function(num_iters: int,
    ...
    Use the following json template [STRICT] to write a draft resume. Within the template, the elements in square brackets contain descriptions of what should be filled within that place. Fill them with the details that fulfil the guidelines while taking into account the information analyzed in the steps before.
    ...
    "skills": [
                "Programming Languages" : a list of 5-10 skills about meaningful programming languages; tailor this information to the candidate and job description"
                "Tools and Technologies" : a list of 5-10 skills about meaningful tools and technologies; tailor this information to the candidate and job description",
                "Soft Skills" : a list of 2-6 skills about meaningful soft skills; tailor this information to the candidate and job description",
    ]
    "a list of 10-20 skills about meaningful professional experience, tools and achievements; tailor this information to the candidate and job description",
    "work_history": [
                {{
                    "job_title": "",
                    "company_name": "",
                    ...
                    "end_date": "",
                    "responsibilities": [
                        # List of bullet points about the experience, tailoring it to the role, company and the candidate, Number of characters should lie between 125 and 129 per bullet point, and the number of bullet points should be between 3 and 5 . [STRICTLY FOLLOW THE CHARACTER RANGE AND NUMBER OF BULLET POINTS, DO NOT CHANGE IT]
                    ]
                }},
                ...
            ],
    "projects": [
        {{
            "project_title": "",
            "points":[
                # List of bullet points about the projects, tailoring it to the role, company and the candidate, Number of characters should lie between 200 - 250 per bullet point, and the number of bullet points should be between 3 and 5. [STRICTLY FOLLOW THE CHARACTER RANGE AND NUMBER OF BULLET POINTS, DO NOT CHANGE IT]
            ],
            "link": ""
        }},
        ...
    ...
    5. Remove any vague,soft, or junior language. Ensure ATS-Critical tools and methods are included in the resume.
    6. Refine phrasing for senior tone, clarity and quantifiable impact.
    </response_rules>
...
def final_function(
    ...
    for i in range(num_iters):
        ...
        print(final_resume)
        resume = final_resume
        feed_ = function_resume_feedback(4, final_resume, job_description, company_name)
        if feed_["score"] >= 95:
            break
...
def gen_Resume(resume:str,job_description:str,urls:List[str],role:str,company_name:str):
    out_ = final_function(3,resume,job_description,urls,role,company_name)
    print(out_)
    out_ = genResume(out_["final_resume"])
    return out_
...
urls = ["https://github.com/Msonu007"]

#print(gen_Resume(resume_str,job_Desc,urls,role,company_name))

#print(get_profile_url_tavily("https://github.com/Msonu007"))
